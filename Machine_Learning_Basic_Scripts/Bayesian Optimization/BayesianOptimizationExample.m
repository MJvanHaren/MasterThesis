...
figure(1)
subplot(2,1,1)
p1 = plot(xBayesian(1:3,1),yBayesian(1:3,1),'o'); hold on;
p2 = plot(linspace(0,1,1000),trueFunction(linspace(0,1,1000)));
xlabel('x coordinate');
ylabel('y value');
legend([p1,p2],{'Samples of function','true function'},'Autoupdate','off');
subplot(2,1,2)
xlabel('x coordinate');
...
for i = 4:N
    ...
    PI = ProbabilityImprovement(xBayesian(1:i-1,1),xTest,yBayesian(1:i-1,1),hyp);
    [~, index] = max(PI); 
...
plot(xGridMax, (yGridMax),'o');
plot(xRandomMax, (yRandomMax),'^');
plot(xBayesianMax,yBayesianMax,'s');
